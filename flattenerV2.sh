#!/bin/bash

# A script to generate a directory structure and a flattened content file from a folder.
#
# Enhancements:
# 1. Prompts user to choose between generating only the structure file or both files.
# 2. Replaces the restrictive top-level exclusion with a flexible pattern-based exclusion
#    that works for any file, directory, or glob pattern anywhere in the tree.

# Set locale to handle binary data and special characters safely.
export LC_ALL=C

# --- 1. SETUP ---
# Check if a folder path was provided as an argument.
if [ $# -eq 0 ]; then
  echo "Error: Please provide a folder path."
  echo "Usage: $0 /path/to/your/project"
  exit 1
fi

# Clean the provided folder path and get its base name.
folder_path="${1%/}"
folder_name=$(basename "$folder_path")
current_datetime=$(date +"%Y-%m-%d_%H-%M-%S")

# Define the output directory to be the same directory where the script is located.
script_dir=$(dirname "$0")
output_dir="$script_dir"

# Define the full paths for the output files.
structure_file="${output_dir}/STRUCTURE_${folder_name}_${current_datetime}.md"
flattened_file="${output_dir}/FLATTENED_${folder_name}_${current_datetime}.md"

# --- 2. USER CHOICES ---
# Ask the user to select the desired output type.
echo "Select output type:"
echo "  [1] Structure file only"
echo "  [2] Structure file AND Flattened content file (default)"
read -p "Enter your choice [1-2]: " output_choice
output_choice=${output_choice:-2} # Default to '2' if the user just presses Enter.

generate_flattened=false
if [[ "$output_choice" == "2" ]]; then
  generate_flattened=true
fi

# Ask the user for directories, files, or glob patterns to exclude.
echo
echo "You can now specify items to exclude from the output."
echo "The patterns work for items anywhere in the directory tree."
echo "Examples:"
echo "  - To exclude a directory and all its contents: node_modules"
echo "  - To exclude all '.log' files: *.log"
echo "  - To exclude a specific subdirectory: src/tests"
echo
# The '-a' flag reads the space-separated input directly into an array.
read -p "Enter items to exclude (space-separated), or press Enter to skip: " -a excluded_patterns

# --- 3. PREPARE EXCLUSION ARGUMENTS ---

# Build exclude arguments for the `tree` command.
# We join all patterns with a '|' for a single, powerful -I argument.
tree_exclude_args=()
if [ ${#excluded_patterns[@]} -gt 0 ]; then
  joined_patterns=$(IFS="|"; echo "${excluded_patterns[*]}")
  tree_exclude_args+=("-I" "$joined_patterns")
fi

# Build exclude arguments for the `find` command.
# This logic is robust for matching directories, files, and globs.
find_exclude_args=()
if [ ${#excluded_patterns[@]} -gt 0 ]; then
    find_exclude_args+=(-not "(")
    for pattern in "${excluded_patterns[@]}"; do
        # Exclude matching directories and their contents, and matching file/glob names.
        find_exclude_args+=(-path "*/$pattern/*" -o -path "*/$pattern" -o -name "$pattern" -o)
    done
    # Remove the trailing '-o' from the last pattern.
    find_exclude_args=("${find_exclude_args[@]:0:${#find_exclude_args[@]}-1}")
    find_exclude_args+=(")")
fi

# --- 4. GENERATE STRUCTURE FILE ---
echo
echo "Generating directory structure..."

# Create the markdown header for the structure file.
echo "# Directory Structure: $folder_name" > "$structure_file"
echo "**Generated:** $(date +'%Y-%m-%d %H:%M:%S')" >> "$structure_file"
echo -e "\n## Folder and File Tree\n" >> "$structure_file"

# Generate the directory tree using the `tree` command if it's available.
if command -v tree &>/dev/null; then
  echo '```' >> "$structure_file"
  # We `cd` into the directory so `tree` produces clean, relative paths.
  (cd "$folder_path" && tree -a --noreport "${tree_exclude_args[@]}") >> "$structure_file"
  echo '```' >> "$structure_file"
else
  # If `tree` is not found, use `find` as a fallback for a basic list.
  echo "Warning: 'tree' command not found. Using 'find' for a basic structure list."
  echo '```' >> "$structure_file"
  (cd "$folder_path" && find . "${find_exclude_args[@]}" | sort) >> "$structure_file"
  echo '```' >> "$structure_file"
fi

# --- 5. GENERATE FLATTENED CONTENT FILE (if requested) ---
if [ "$generate_flattened" = true ]; then
  echo "Generating flattened content file..."

  # Create the markdown header for the flattened content file.
  echo "# Flattened Content: $folder_name" > "$flattened_file"
  echo "**Generated:** $(date +'%Y-%m-%d %H:%M:%S')" >> "$flattened_file"
  
  # Find all files to be processed, respecting exclusions.
  # Using -print0 and a while/read loop is the safest way to handle filenames
  # with spaces or special characters.
  files_to_process=()
  while IFS= read -r -d $'\0' file; do
      files_to_process+=("$file")
  done < <(find "$folder_path" "${find_exclude_args[@]}" -type f -print0)

  # Check if any files were found after applying exclusions.
  if [ ${#files_to_process[@]} -eq 0 ]; then
      echo "No files found to process after applying exclusions."
      echo -e "\n## File Contents\n\nNo text files found to include." >> "$flattened_file"
  else
      # Add summary and file type statistics to the STRUCTURE file.
      echo -e "\n## Summary\n" >> "$structure_file"
      echo "Total files included in flattened output: ${#files_to_process[@]}" >> "$structure_file"
      echo -e "\n## Included File Types\n" >> "$structure_file"
      echo '```' >> "$structure_file"
      # This pipeline counts the occurrences of each file extension.
      for file in "${files_to_process[@]}"; do
        filename=$(basename "$file")
        extension="${filename##*.}"
        if [[ "$filename" == "$extension" ]]; then
            echo "(no extension)"
        else
            echo "$extension"
        fi
      done | sort | uniq -c | sort -rn >> "$structure_file"
      echo '```' >> "$structure_file"

      # Begin writing the main content to the flattened file.
      echo -e "\n## File Contents\n" >> "$flattened_file"
      
      for file in "${files_to_process[@]}"; do
        relative_path="${file#$folder_path/}"
        filename=$(basename "$file")
        extension="${filename##*.}"
        lang="text"
        display_content=1

        # Determine language for syntax highlighting and filter out binary files.
        case "$extension" in
          js)   lang="javascript" ;;
          tsx)  lang="typescript" ;;
          ts)   lang="typescript" ;;
          json) lang="json" ;;
          md)   lang="markdown" ;;
          txt)  lang="text" ;;
          svg)  lang="xml" ;;
          html) lang="html" ;;
          css)  lang="css" ;;
          py)   lang="python" ;;
          java) lang="java" ;;
          rb)   lang="ruby" ;;
          sh|bash|zsh) lang="bash" ;;
          php)  lang="php" ;;
          c)    lang="c" ;;
          cpp)  lang="cpp" ;;
          h)    lang="c" ;;
          hpp)  lang="cpp" ;;
          go)   lang="go" ;;
          rs)   lang="rust" ;;
          sql)  lang="sql" ;;
          yaml|yml) lang="yaml" ;;
          xml)  lang="xml" ;;
          # Common binary image types
          png|jpg|jpeg|gif|ico|webp|bmp|tiff)
            echo -e "\n### \`$relative_path\`\n" >> "$flattened_file"
            echo "**Binary image file (content not displayed)**" >> "$flattened_file"
            display_content=0
            ;;
          # Other common binary/unsuitable types
          pdf|doc|docx|xls|xlsx|ppt|pptx|zip|tar|gz|rar|bin|exe|dll|so|dylib|o|a|class|jar|war|ear|mp3|mp4|mov|avi)
            echo -e "\n### \`$relative_path\`\n" >> "$flattened_file"
            echo "**Binary file (content not displayed)**" >> "$flattened_file"
            display_content=0
            ;;
          *)
            # As a final check, use the `file` command to detect if a file is text-based.
            if ! file -bL --mime-encoding "$file" | grep -q 'us-ascii\|utf-8'; then
              echo -e "\n### \`$relative_path\`\n" >> "$flattened_file"
              echo "**Binary file detected (content not displayed)**" >> "$flattened_file"
              display_content=0
            fi
            ;;
        esac

        # If the file was marked as binary, skip to the next one.
        [ "$display_content" -eq 0 ] && continue

        # Add the file's content to the flattened markdown file.
        echo -e "\n### \`$relative_path\`\n" >> "$flattened_file"
        echo '```'"${lang}" >> "$flattened_file"
        # Safely `cat` the file content, replacing triple-backticks to avoid breaking markdown code blocks.
        cat "$file" | LC_ALL=C sed 's/```/`` `/g' 2>/dev/null >> "$flattened_file"
        echo -e '\n```' >> "$flattened_file"
      done
  fi
fi

# --- 6. FINAL MESSAGES ---
echo
echo "---"
echo "✅ Success!"
echo "Directory structure written to: $structure_file"
if [ "$generate_flattened" = true ]; then
  echo "Flattened content written to: $flattened_file"
fi